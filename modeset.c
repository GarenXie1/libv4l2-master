...
static void drm_show_img(unsigned int width, unsigned int height, unsigned char* rgb32);
...
void fb_show_img(unsigned int width, unsigned int height, unsigned char* rgb32)
{
	...
	DEBUG("m_width -> %d ; m_height -> %d",m_width,m_height);
    for(y = 0; y < m_height; ++y)	// 480
    {
        for(x = 0; x < m_width; ++x)	// 640
        {
            color = *(prgb32+y*width+x);
			//DEBUG("x -> %d ; y -> %d",x,y);
            fb_put_pixel(x, y, 0x000000FF);	//0x00AAFFFF
        }
    }
}
	...
	for (i = 0; i < 50000; ++i) {
		...
		for (iter = modeset_list; iter; iter = iter->next) {
			...
			for (j = 0; j < iter->height; ++j) {
				fprintf(stderr, "iter->height -> %d\n",iter->height);	// 1920
				fprintf(stderr, "iter->width -> %d\n",iter->width); // 1200
				fprintf(stderr, "iter->stride -> %d\n",iter->stride);	// 4800
